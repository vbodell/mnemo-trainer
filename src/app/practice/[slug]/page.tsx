...
function DeckOfCardsGame() {
  const [phase, setPhase] = useState<'intro' | 'memorize' | 'recall' | 'eval'>(
    'intro'
  )
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-8 lg:p-24">
      ...
      {phase === 'memorize' && (
        <div
          className="flex flex-col items-center justify-center h-full w-full flex-grow cursor-pointer"
          onClick={nextCard}
        >
          ...
          <div className="flex flex-col items-center justify-center border-4 border-gray-300 rounded-xl w-64 h-96 bg-white shadow-2xl select-none hover:scale-105 transition-transform">
            <span
              className={classNames(
                'text-6xl font-bold',
                getCardColor(deck[currentIndex])
              )}
            >
      ...
      {phase === 'recall' && (
        <form
          ...
          className="flex flex-col gap-6 w-full md:w-2/3 lg:w-1/2 h-full"
        >
